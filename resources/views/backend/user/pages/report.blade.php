...
@section('title', 'Plagiarismchecker | Plagiarism Report')
@section('description', "Plagiarismchecker | Plagiarism Report")
@section('content')
<style type="text/css">
...
<input id = "report_id" value="{{$report_id}}" hidden>
	...
	<div class="col-lg-8 col-md-12 col-sm-12">
		...
		<div class="card">
		    <div class="card-header card-header-success card-header-icon">
              	<div class="row">
		      <h4 class="card-title col-lg-8 col-md-8 col-sm-12" style="padding-left: 50px;">{{$file_name}}<span class="simiar-percent" style="margin-left: 20px"></span></h4>
				<div class="col-lg-4 col-md-4 col-sm-12 text-right" >
                    <a rel="tooltip" class="btn btn-warning btn-round" data-original-title="Download Report" title="Download Report" style="color: white" download="report" id = 'report-download-link'>
                              <i class="material-icons">save_alt</i>
                    </a>
                    <button type="button" rel="tooltip" class="btn btn-rose btn-round" data-original-title="Delete Report" title="Delete Report" onclick="deleteReport('{{$report_id}}')">
...
<script type="text/javascript">
	...
	$( document ).ready(function() {
	    ...
	    if(status == 0)
	    {
     		swal({ title:"Error!", text: "This report is processing now", type: "error", buttonsStyling: false, confirmButtonClass: "btn btn-success"}).then((result) => {
	    ...
	    else if(status == 1)
	    {
	    	...
	    	$.ajax({
                  ...
                  {
                      $('#document-content').html(response1.data);
                      if(response1.status == 'success')
	              		{
	              			setToOriginalStatus();
              	...
              	error: function(jqXhr1, textStatus1, errorThrown1) {
	                    ...
	                    swal({ title:"Error!", text: 'Error during loading', type: "error", buttonsStyling: false, confirmButtonClass: "btn btn-success"}).then((result) => {
							      window.location.href = "{{ route('user.newsearch') }}";
							});
	                }
                  });
	    ...
	    { 			
              ...
              success: function(response){
                  ...
                  {
                  	swal({ title:"Error!", text: response.reason, type: "error", buttonsStyling: false, confirmButtonClass: "btn btn-success"}).then((result) => {
					      window.location.href = "{{ route('user.newsearch') }}";
					});
                  }
              },
              error : function(jqXhr, textStatus, errorThrown) {
                    swal({ title:"Error!", text: 'Network error', type: "error", buttonsStyling: false, confirmButtonClass: "btn btn-success"}).then((result) => {
					      window.location.href = "{{ route('user.newsearch') }}";
					});
                }
            });
...
function myfunction(response1)
{
			...
			swal({ title:"Plagiarism rate is " + response1.score + "%", 
					type: "success", 
			...
			swal({ title:"Plagiarism rate is " + response1.score + "%", 
					...
					html: "We hope you like our checker. It is free to use, so we would appreciate if you tell your friends on social media about us and place a link to plagiarismchecker.eu. Or you can pay the amount you wish<br/>" + 
					"<button class = 'btn btn-primary' id = 'facebookbutton'>Facebook</button><button class = 'btn btn-success' id = 'paypalbutton'>Purchase</button><button class = 'btn btn-danger' id = 'cancelbutton'>Cancel</button>",
					onOpen: function (dObj) {
						...
						$('#paypalbutton').on('click',function () {
							swal.close();
							var report_id_value = $("#report_id").val();
							document.location.href = '{{ route("user.payforsupport", ["report_id" => '']) }}' + '/' + report_id_value;
